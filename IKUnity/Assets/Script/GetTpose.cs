...
public class GetTpose : MonoBehaviour
{
    public float offset;//存储T_pose前让脚触地，存储完毕后再将ref置为0
    public GameObject cha_ref;
    public Quaternion[] T_R = new Quaternion[6];//不能直接给Transform赋值！！！中间会一直错！！
    public Vector3[] T_P = new Vector3[6];
    ...
    {
        cha_ref.GetComponent<Transform>().position = new Vector3(0.0f, offset, 0.0f);
        for (int i = 0; i < 6; i++)
        {
        ...
        cha_ref.GetComponent<Transform>().position = new Vector3(0.0f, 0.0f, 0.0f);
